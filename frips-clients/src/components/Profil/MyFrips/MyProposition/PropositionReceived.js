...
const renderStatus = ({ dateApprove, Approve, id_Account, id }, dispatch) => {
    ...
    return (
      <Box display={"flex"} flexDirection="column">
        ...
        <Typography style={{ fontSize: 16 }}>
          L'utilisateur a 24 heures pour faire le paiement autrement l'offre
          sera annulée
        </Typography>
      </Box>
    );
    ...
    return (
      <React.Fragment>
        <Button
          ...
          onClick={() => {
            dispatch(
              sendStatusProposition(id, new Date(), true, id_Account, true)
            );
          }}
        >
          Accepter
        ...
        <Button
          onClick={() => {
            dispatch(sendStatusProposition(id, new Date(), false, id_Account, true));
          }}
          variant="contained"
          color="primary"
          style={{ marginLeft: 5 }}
...
const PropositionReceived = ({ loading, item, msg }) => {
...
const mapStateToProps = (state) => ({
  ...
  item: state.myFrips.propostionReceived,
  myaccount: state.auth.user,
  msg: state.myFrips.msg,
});

export default connect(mapStateToProps)(PropositionReceived);
